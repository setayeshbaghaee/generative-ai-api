# Generative AI API 

A collection of hands-on experiments with Generative AI APIs and multimodal applications in Python.

## What is included?

- OpenAI-compatible chat APIs
- Cohere chat API
- OpenRouter / multimodal model calls
- Text-to-Speech and Speech-to-Text
- Image generation and image understanding
- LangChain examples
- Gradio apps for chat, vision, and speech

## Setup

Set the required API keys as environment variables before running the notebook:

```bash
OPENAI_API_KEY=your_key_here
COHERE_API_KEY=your_key_here
AVALAI_API_KEY=your_key_here
OPENROUTER_API_KEY=your_key_here
```

Some examples also use local files such as `sample.png` and `sample_audio.m4a`.

## Learning Context

This repository contains cleaned and organized hands-on exercises completed while studying Generative AI APIs.  
